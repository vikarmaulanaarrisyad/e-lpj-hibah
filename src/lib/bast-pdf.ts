...
export interface ExportBastOptions {
  elementId?: string;
  nomorBast?: string;
  formData?: BastFormData;
...
/**
 * Menangkap elemen Berita Acara (BAST) menjadi gambar PNG resolusi tinggi (~300 DPI)
 */
async function captureBastImage(
  element: HTMLElement,
  options: ExportBastOptions
): Promise<string> {
...
/**
 * Ekspor Berita Acara (BAST) ke format PDF Kertas F4 / Folio Portrait (215mm x 330mm)
 * Dilengkapi margin kiri ekstra (~28mm) untuk kebutuhan penjilidan / jilid dokumen LPJ
 */
export async function exportBastToPdf(options: ExportBastOptions = {}): Promise<boolean> {
  const elementId = options.elementId || "bastPrintArea";
  const element = document.getElementById(elementId);

  if (!element) {
    swalError("Elemen Tidak Ditemukan", "Format dokumen Berita Acara (BAST) belum siap untuk diekspor.");
    return false;
  }
  ...
  try {
    swalLoading(
      ...
      "Merender Berita Acara (BAST) presisi F4 Portrait (215mm × 330mm) dengan Margin Jilid..."
    );
    ...
    const imgData = await captureBastImage(element as HTMLElement, options);
    ...
    if (imgHeight <= pageHeight) {
      ...
      // Penanganan multi-halaman jika rincian barang sangat banyak
    ...
    let cleanNoBast = (options.nomorBast || options.formData?.nomorBast || "BAST")
      .trim()
      .replace(/[/\\?%*:|"<>.]/g, "_")
      .replace(/\s+/g, "_")
      .replace(/_+/g, "_");

    if (cleanNoBast.toLowerCase().endsWith("_pdf")) {
      cleanNoBast = cleanNoBast.slice(0, -4);
    }
    if (cleanNoBast.toLowerCase().endsWith(".pdf")) {
      cleanNoBast = cleanNoBast.slice(0, -4);
    }

    const filename = `BAST_${cleanNoBast}_F4.pdf`;
        ...
        <div class="text-left space-y-2 mt-2 text-xs text-slate-300">
          <div class="bg-slate-900/90 p-3 rounded-xl border border-slate-800 space-y-1.5">
            ...
            <div class="pt-1.5 border-t border-slate-800 flex items-center justify-between text-[11px]">
              ...
              <span class="font-semibold text-white bg-slate-800 px-2 py-0.5 rounded">Berita Acara (BAST) (.pdf)</span>
            </div>
    ...
    console.error("Gagal mengekspor PDF Berita Acara (BAST):", error);
    swalClose();
    swalError(
      "Gagal Ekspor PDF",
      error?.message || "Terjadi kesalahan saat memproses Berita Acara menjadi PDF."
    );
    return false;
  }
}
